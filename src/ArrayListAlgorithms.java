...
public class ArrayListAlgorithms
{
    ...
    public static void wackyVowels(ArrayList<String> wordList)
    {
        String aeio = "aeio";
        for(int i = 0; i < wordList.size(); i++) {
            for(int j = 0; j < aeio.length(); j++) {
                if(wordList.get(i).contains(aeio.substring(j, j+1))) {
                    wordList.remove(i);
                    i--;
                    break;
                }
    ...
    {
        int originalWordListSize = wordList.size();
        for(int i = 0; i < originalWordListSize; i++) {
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence)
    {
        ...
        while(sentence.contains(" ")) {
            words.add(0,sentence.substring(0, sentence.indexOf(" ")));
            sentence = sentence.substring(sentence.indexOf(" ") + 1);
        }
        words.add(0,sentence);
        return words;
    }
    ...
    {
        for(int i = wordList.size() - 1, loops = 0; i >= 0 && loops < wordList.size(); i--, loops++) {
